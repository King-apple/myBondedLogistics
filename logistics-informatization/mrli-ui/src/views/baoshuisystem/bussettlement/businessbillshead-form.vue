<template>
  ...
  >
    ...
    >
      <el-row>
        <el-col :span="8">
          <el-form-item label="合同编号" prop="contCod" style="width: 280px">
            <el-select
              clearable
      ...
      <el-row>
        <el-col :span="8">
          ...
          >
            <el-input
              v-model.lazy="dataForm.feeQuantity"
              @input="oninputThree"
              placeholder="计费数量"
            ></el-input>
          </el-form-item>
      ...
      <el-row>
        ...
        <el-col :span="8">
          <el-form-item label="结算单位" prop="secondNam" style="width: 280px">
            <el-input
      ...
      <el-card class="box-card">
        ...
        <div v-for="(accountsListItem, index) of accountsList" :key="index">
          <div style="display: flex">
            ...
            <el-form-item label="费率单位" style="width: 260px" prop="feeUnit">
              <el-select
            ...
            <el-form-item label="税率" style="width: 260px">
              <el-select
                v-model="accountsListItem.taxRate"
                placeholder="请选择"
                filterable
              >
                <el-option
                  v-for="(item, feeIndex) in taxRateOptions"
                  :key="feeIndex"
                  :label="item.label"
                  :value="item.value"
                >
        ...
        <div slot="header" class="clearfix">
          ...
          <div v-for="(advanceListItem, index) of advanceList" :key="index">
            <div style="display: flex">
              ...
              <el-form-item label="税率" style="width: 270px">
                <el-select
                  v-model="advanceListItem.taxRate"
                  placeholder="请选择"
                  filterable
                >
                  <el-option
...
import { getTaxRate } from "@/api/baoshuisystem/getdictdata";
...
export default {
      ...
      handler(newVal, oldVal) {
        ...
        for (let index = 0; index < arrlen; index++) {
          ...
          if (obj2 != null && obj2[index] != null && obj[index].feeQuantity != obj2[index].feeQuantity) {
            this.advanceList[index].totalMoney = (
              Number(newVal[index].feeRat) * Number(newVal[index].feeQuantity)
            ).toFixed(2);
            // this.advanceList[index].taxMoney = (
            //   (this.advanceList[index].totalMoney / (Number(this.advanceList[index].taxRate) + 1)) *
            //   this.advanceList[index].taxRate
            // ).toFixed(2);
            // let temp = this.advanceList[index].totalMoney - this.advanceList[index].taxMoney;
            // this.advanceList[index].money = temp.toFixed(2);
          }
    ...
    accountsDataNew: {
      ...
      handler(newVal, oldVal) {
        ...
        for (let index = 0; index < arrlen; index++) {
          //只改变数量
          if (obj2 != null && obj2[index] != null && obj[index].feeQuantity != obj2[index].feeQuantity ) {
            this.accountsList[index].totalMoney = (
              Number(obj[index].feeRat) * Number(obj[index].feeQuantity)
            ).toFixed(2);
            // this.accountsList[index].taxMoney = (
            //   (this.accountsList[index].totalMoney / (Number(this.accountsList[index].taxRate) + 1)) *
            //   this.accountsList[index].taxRate
            // ).toFixed(2);
            // let temp = this.accountsList[index].totalMoney - this.accountsList[index].taxMoney;
            // this.accountsList[index].money = temp.toFixed(2);
          }
          if(obj2[index] !== undefined){
               this.accountsList[index].taxMoney = (
              (this.accountsList[index].totalMoney / (Number(this.accountsList[index].taxRate) + 1)) *
  ...
  methods: {
    ...
    getCount(data) {
      ...
      return this.countMoney == (""||XEUtils.isNaN(this.countMoney)) ? 0 : temp.toFixed(2)
    },
    getCount2(data) {
      let temp = 0;
      data.forEach(res => {
      ...
      return this.countMoney2 == (""||XEUtils.isNaN(this.countMoney2)) ? 0 : temp.toFixed(2)
    },
    getTaxMoneyCount(data) {
      ...
      return temp == (""||XEUtils.isNaN(this.temp)) ? 0 : temp.toFixed(2)
    },
    ...
    init(id) {
      ...
      getTaxRate().then(resp => {
        this.taxRateOptions = resp.data.data
      })
      //通过id判断新增还是修改
      if (id == undefined) {
        this.accountsList = [];
    ...
    accountsTwo(event, index) {
      ...
      this.accountsList[index].totalMoney = this.accountsList[
        index
    ...
    oninputTwoAccountsListTaxMoney(event, index) {
      ...
      this.accountsList[index].taxMoney = this.accountsList[
        index
    ...
    getCompanyName() {
      ...
      // getCompany(61).then((response) => {
      //   this.rateStatic = response.data.data.paramValue;
      //   this.don = Number(response.data.data.paramValue) + 1;
      // });
    },
    addFee: function () {
      this.accountsList.push({
        ...
        feeRat: 0,
        taxRate:0,
        feeQuantity: this.dataForm.feeQuantity == "" ? 0 : this.dataForm.feeQuantity,
      });
    },
    addAccount() {
      ...
      this.$refs["dataForm"].validate((valid) => {
        ...
        if (valid) {
          ...
          if (this.dataForm.fId) {
            ...
            // this.$set(this.dataFrom, "inputType", 0);
            this.dataForm.inputType = 0;
            addObj(this.dataForm)
              .then((data) => {
                //文件添加
                for (var i in this.fileList) {
                  this.fileList[i].tableName = 'business_bills_head'
    ...
    getFileInfoUpdate: function () {
      var map = { tableName: 'business_bills_head', tableFid: this.dataForm.fId };
