<template>
  <div class="mod-config">
    <basic-container>
      ...
      >
        <el-form-item label="入驻企业名称" prop="settledCompany">
          <el-input
        ...
        <el-form-item label="作业委托人" label-width="82px" prop="consignNam">
          <el-input
        ...
        <el-form-item
          label="报关单号维护状态"
          label-width="150px"
          prop="inCustomsDeclarationStatus"
        >
          <el-select
            v-model="dataForm.inCustomsDeclarationStatus"
            placeholder="报关单号维护状态:"
            filterable
            style="width: 10vw"
          >
            <el-option
              v-for="item in inCustomsDeclarationStatusList"
              :key="item.fId"
              :label="item.label"
              :value="item.value"
            >
        ...
        <el-form-item label="业务类型" prop="businessKnd">
          <el-select
          ...
          >
            <el-option
              v-for="item in businessKndOptions"
              :key="item.label"
              :label="item.label"
              :value="item.value"
            >
        ...
        <el-form-item>
          <el-button
            v-if="permissions.inoutyardheadCheckOut_add"
            icon="el-icon-plus"
            type="primary"
        ...
        <el-form-item>
          <el-button
            v-if="permissions.inoutyardheadCheckOut_export"
            type="warning"
            icon="el-icon-download"
            :disabled="single"
            @click="downloadFile('保税物流中心出库单', '模板')"
            >导出</el-button
          >
        </el-form-item>
        <el-form-item>
          <el-button
            v-if="permissions.inoutyardhead_export"
            icon="el-icon-info"
            type="primary"
            :disabled="single"
            @click="expoprt('保税物流中心出库单', '模板')"
            >查看</el-button
          >
        </el-form-item>
        ...
        <el-form-item>
          <el-upload
            v-if="permissions.inoutyardheadCheckOut_import"
            :headers="headers"
            :show-file-list="false"
            :on-success="uploadSuccess"
            class="avatar-uploader"
            action="/baoshuisystem/excelUploadDownload/uploadForInoutyardBodyAndHead"
            :on-progress="onProgress"
            :on-change="onChange"
            v-loading="fileLoading"
          >
            <el-button size="small" icon="el-icon-upload2" type="success"
              >导入</el-button
            >
          </el-upload>
        </el-form-item>
        ...
        <el-form-item>
          <el-button
            v-if="permissions.inoutyardheadCheckOut_Busi"
            type="danger"
            size="small"
            :disabled="single"
            icon="el-icon-tickets"
            @click="addOrUpdateBusinessHandle('保税物流中心出库单', '模板')"
            >业务审核</el-button
          >
        </el-form-item>
        <!--<el-form-item>
              <el-button
                v-if="permissions.inoutyardheadCheckOut_Oper"
                type="danger"
                size="small"
                :disabled="single"
                icon="el-icon-tickets"
                @click="addOrUpdateOpearHandle()"
                >操作审核</el-button
              >
        </el-form-item>-->
      </el-form>
        ...
        >
          ...
          >
            <template slot-scope="scope">
              <div slot="reference" class="name-wrapper">
                <el-tag
                  :type="scope.row.businessCheckStatus | getbusinessCheckStatus"
                  close-transition
                  >{{
                    scope.row.businessCheckStatus | getbusinessCheckStatusText
                  }}</el-tag
                >
              </div>
            </template>
          ...
          <!--<el-table-column prop="settledCompany" header-align="center" align="center" label="入驻企业名称">
					</el-table-column>
					<el-table-column prop="inOut" header-align="center" align="center" label="出入库">
					</el-table-column>
					<el-table-column prop="inoutYardKnd" header-align="center" align="center" label="单据类型">
					</el-table-column>
					<el-table-column prop="businessKnd" header-align="center" align="center" label="业务类型">
					</el-table-column>
					<el-table-column prop="inoutYardNam" header-align="center" align="center" label="单据名称">
					</el-table-column>
					<el-table-column prop="inCategory" header-align="center" align="center" label="入区类别">
					</el-table-column>
					<el-table-column prop="consignCod" header-align="center" align="center" label="货主代码">
					</el-table-column>
					<el-table-column prop="outCheckNum" header-align="center" align="center" label="出区核注清单号">
					</el-table-column>
					<el-table-column prop="outCategory" header-align="center" align="center" label="出区类别">
					</el-table-column>
					<el-table-column prop="operNam" header-align="center" align="center" label="添加人">
					</el-table-column>
					<el-table-column prop="recordTim" header-align="center" align="center" label="添加时间">
					</el-table-column>
					<el-table-column prop="updateNam" header-align="center" align="center" label="修改人">
					</el-table-column>
					<el-table-column prop="updateTim" header-align="center" align="center" label="修改时间">
					</el-table-column>
					<el-table-column prop="delNam" header-align="center" align="center" label="删除人">
					</el-table-column>
					<el-table-column prop="delTim" header-align="center" align="center" label="删除时间">
					</el-table-column>
					<el-table-column prop="customsBrokerCod" header-align="center" align="center" label="报关代理企业代码">
					</el-table-column>
					<el-table-column prop="settledCod" header-align="center" align="center" label="入驻企业字母代码">
					</el-table-column>
					<el-table-column prop="warehouseCod" header-align="center" align="center" label="仓库代码">
					</el-table-column>-->
          <el-table-column
            header-align="center"
            min-width="170px"
          ...
          >
            <template slot-scope="scope">
              <el-button
                ...
                v-show="
                  scope.row.businessCheckStatus == '0' ||
                  scope.row.businessCheckStatus == '2'
                "
                >修改</el-button
              >
              <el-button
                ...
                size="small"
                icon="el-icon-delete"
                @click="deleteHandle(scope.row.fId, scope.row)"
                >删除</el-button
              >
              <el-button
                v-if="permissions.inoutyardbody_view"
                type="text"
                size="small"
                icon="el-icon-plus"
                @click="openyardBody(scope.row)"
                >表体</el-button
              >
              <!--<el-button
      ...
      <!-- 弹窗, 新增 / 修改 -->
      <table-business-form
        v-if="addOrUpdateBusinessVisible"
        ref="addOrBusinessUpdate"
        @refreshDataList="getDataList"
      ></table-business-form>

      <!-- 弹窗, 新增 / 修改 -->
      <table-opear-form
        v-if="addOrUpdateOpearVisible"
        ref="addOrOpearUpdate"
        @refreshDataList="getDataList"
      ></table-opear-form>
    </basic-container>
    ...
    <el-dialog title="业务审核" :visible.sync="BUsinessVisible" width="90%">
      <div
        class="table-html-wrap"
        v-html="tableBUsinessHtml"
        style="overflow-x: auto"
        id="printBusinessTest"
      ></div>
      <div style="margin-top: 5%; margin-left: 80%">
        <el-button @click="BUsinessVisible = false">取 消</el-button>
        <el-button type="primary" @click="aduitSubmit(1)" v-if="canSubmit"
          >审核通过</el-button
        >
        <el-button type="danger" @click="aduitSubmit(2)" v-if="canSubmit"
          >审核不通过</el-button
        >
      </div>
    </el-dialog>
    ...
    <el-dialog
      title="查看"
      :visible.sync="dialogVisibleExportSecond"
      width="20%"
    >
      <span>选择要查看的模板</span>
      <el-form :model="secondForm">
...
export default {
  ...
  methods: {
    ...
    getDataList() {
      ...
      this.dataForm.inOut = "0" //出库单查询
      this.dataForm.inoutYardKnd = "保税物流中心出库" //单据类型
