  ...
  <div class="mod-config">
    <basic-container>
      ...
      >
        ...
        <el-form-item label="入库日期" prop="params.recordDte">
          <el-date-picker
        ...
        <div style="margin-bottom:10px;font-weight:bold;">
          <span style="font-size: 16px">合计：入库量：{{countDataList.length == isNaN(countDataList) ? 0 : countDataList.weightWgt }} 吨&nbsp;&nbsp;&nbsp;&nbsp;
            入库件数 ：{{ countDataList == isNaN(countDataList) ? 0 : countDataList.piecesNum }}件&nbsp;&nbsp;&nbsp;&nbsp;
          </span>
          <span style="font-size: 16px" >出库量：{{ countDataList.length == isNaN(countDataList) ? 0 : outAmount}}吨&nbsp;&nbsp;&nbsp;&nbsp;
            出库件数 ：{{
              countDataList.length == isNaN(countDataList) ? 0 :outNumber
            }}
            件&nbsp;&nbsp;&nbsp;&nbsp;</span>
            <span style="font-size: 16px" >库存量：{{ countDataList.length == isNaN(countDataList) ? 0 : countDataList.surplusWeight }}吨&nbsp;&nbsp;&nbsp;&nbsp;
            库存件数 ：{{ countDataList.length == isNaN(countDataList)? 0 : countDataList.surplusPieceNum }}件
            </span>
        </div>
...
<script>
import {
  fetchList,getIoYard , delObj, getInoutIdDic, getTradeIdDic,
...
export default {
  ...
  methods: {
    ...
    getIoyardDataList() {
      ...
      this.getDataList()
      getIoYard(this.dataForm).then(
        response => {
          this.countDataList = response.data.data
          this.outAmount = this.countDataList.surplusWeight-this.countDataList.weightWgt
          this.outNumber = this.countDataList.surplusPieceNum-this.countDataList.piecesNum
        }
      )
    },
    outPieceFormatter(row) {
      var pieceNum = 0;
      if (row.surplusPieceNum != null) {
        pieceNum = row.surplusPieceNum
      }
      return row.piecesNum - pieceNum
    },
    outWeightFormatter(row) {
      var weight = 0.00;
      if (row.surplusWeight != null) {
        weight = Number(row.surplusWeight)
      }
      return row.weightWgt - weight
    }
  }
}
</script>
