  ...
  export default {
    ...
    created() {
      ...
      /*this.$notify({
        ...
        title: '重要通知',
        duration: 0,
        message:'因HW原因，会对各个用户的信息进行安全检索，请确保个人密码复杂程度，尽量设置高复杂的密码，如有检测出密码简单情况，个人负责。修改个人密码操作如下：点击右上角个人头像旁箭头——>选择个人信息——>主页面密码管理，修改密码即可',
      });*/
    },
    destroyed() {
    ...
    methods: {
      ...
      connection2() {
        ...
        // 建立连接对象
        this.socket = new SockJS('/admin/mrlimessage')// 连接服务端提供的通信接口，连接以后才可以订阅广播消息和个人消息
        // 获取STOMP子协议的客户端对象
        this.stompClient = Stomp.over(this.socket)
        this.stompClient.debug = null
        // 向服务器发起websocket连接
        this.stompClient.connect(headers, () => {
          this.stompClient.subscribe('/task/'+this.userInfo.username + '-' + TENANT_ID+'/topic/notify', (msg) => { // 订阅服务端提供的某个topic;
            this.$notify({
              title: '业务提醒',
              type: 'warning',
              dangerouslyUseHTMLString: true,
